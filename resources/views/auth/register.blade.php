...
@section('content')
    <x-page-section fullh="true">
        <form class="w-80 mx-auto shadow-md p-5 rounded-md" method="post" action="{{route('register_do')}}">
            ...
            <x-text-component type="tel" name="phone" placeholder="Телефон*" value="{{old('phone')}}"></x-text-component>
            <x-text-component type="email" name="email" placeholder="e-mail*" value="{{old('email')}}"></x-text-component>
